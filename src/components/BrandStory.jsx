    ...
    <section id="story" className="relative z-20 min-h-screen overflow-hidden border-t border-black/5 bg-transparent py-36">
      ...
      <div className="mx-auto grid max-w-7xl grid-cols-1 items-center gap-16 px-6 lg:grid-cols-12">
        <div className="lg:col-span-7">
          <motion.p initial={{ opacity: 0, y: 22 }} whileInView={{ opacity: 0.6, y: 0 }} viewport={{ once: true }} transition={{ duration: 1 }} className="text-[12px] font-bold uppercase tracking-[0.48em] text-black">
            The Heritage of Purity
          </motion.p>
          <motion.h2 initial={{ opacity: 0, y: 38, filter: 'blur(8px)' }} whileInView={{ opacity: 1, y: 0, filter: 'blur(0px)' }} viewport={{ once: true }} transition={{ duration: 1.25, delay: 0.12, ease: [0.16, 1, 0.3, 1] }} className="mt-5 font-heading text-6xl font-bold leading-[1.06] text-black sm:text-7.5xl">
            A couture film for scent, light, and memory.
          </motion.h2>
          <div className="mt-8 h-px w-16 bg-gold" />
          <motion.div initial={{ opacity: 0, y: 28 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 1.15, delay: 0.3 }} className="mt-10 max-w-2xl space-y-6 text-base leading-8 text-black/62">
            <p>
            ...
            <p className="border-l-2 border-gold bg-white/20 py-2 pl-6 font-heading text-2xl italic leading-8 text-black backdrop-blur-sm">
        ...
        <motion.div initial={{ opacity: 0, scale: 0.94 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true, margin: '-120px' }} transition={{ duration: 1.35, ease: [0.16, 1, 0.3, 1] }} className="relative mx-auto aspect-[4/5] w-full max-w-[380px] border border-black/10 bg-white/20 p-3 shadow-[0_32px_90px_rgba(0,0,0,0.055)] backdrop-blur-xl lg:col-span-5">
          ...
          <div className="absolute bottom-8 left-8 right-8 border border-white/35 bg-white/30 p-5 text-center backdrop-blur-lg">
            ...
            <span className="mt-2 block text-[11px] font-bold uppercase tracking-[0.32em] text-gold">The Sense of Majesty</span>
          </div>
        </motion.div>
      </div>
    </section>
