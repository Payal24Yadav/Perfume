    ...
    <section id="featured" className="relative z-20 min-h-[115vh] overflow-hidden bg-transparent py-36">
      ...
      <div className="mx-auto grid max-w-7xl grid-cols-1 items-center gap-14 px-6 lg:grid-cols-12">
        ...
        <div className="relative z-30 lg:col-span-6">
          <motion.p initial={{ opacity: 0, y: 22 }} whileInView={{ opacity: 0.62, y: 0 }} viewport={{ once: true }} transition={{ duration: 1 }} className="text-[10px] font-bold uppercase tracking-[0.48em] text-black">
            Olfactory Masterpiece
          </motion.p>
          <motion.h2 initial={{ opacity: 0, y: 34, filter: 'blur(8px)' }} whileInView={{ opacity: 1, y: 0, filter: 'blur(0px)' }} viewport={{ once: true }} transition={{ duration: 1.2, delay: 0.12, ease: [0.16, 1, 0.3, 1] }} className="mt-5 font-heading text-5xl font-bold leading-tight text-black sm:text-6xl">
            Reflections reveal the formula.
          </motion.h2>
          <p className="mt-7 max-w-xl text-sm leading-8 text-black/60">
            ...
            {notes.map(([phase, ingredients], index) => (
              ...
              >
                <div className="flex items-center justify-between gap-4">
                  <h3 className="font-heading text-2xl font-semibold text-black">{phase}</h3>
                  <span className="text-[9px] font-bold uppercase tracking-[0.28em] text-gold">Phase 0{index + 1}</span>
                </div>
                <p className="mt-3 text-xs font-semibold uppercase leading-6 tracking-[0.16em] text-black/65">{ingredients}</p>
              </motion.div>
            ))}
          </div>
        </div>
